...
class PaycoConfirmationModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess(){
    	try {
    		...
    		echo json_encode(['success' => true, 'message' => 'Confirmaci贸n procesada']);
    		exit;
    		
    	} catch (Throwable $e) {
    		$errorMessage = "[" . date('Y-m-d H:i:s') . "] Error: " . $e->getMessage() . " | Archivo: " . $e->getFile() . " | L铆nea: " . $e->getLine() . "\n" . $e->getTraceAsString() . "\n\n";
    		file_put_contents(_PS_MODULE_DIR_ . 'payco/logs/confirmation_error.log', $errorMessage, FILE_APPEND);
    		
    		http_response_code(500);
    		echo json_encode(['success' => false, 'error' => 'Error al procesar confirmaci贸n', 'details' => $e->getMessage()]);
    		exit;
    	}
    }
}