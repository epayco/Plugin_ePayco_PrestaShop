...
var form_ticket_header = document.querySelector("#module_form_4 .panel .panel-heading");
form_ticket_header.style.height = "auto";
form_ticket_header.style.color = "red";
form_ticket_header_prepend.innerHTML = "<ul class='mp-checkout-list'>\
    <li><span>{l s='Offer cash payments.' mod='payco'}</span></li>\
    <li><span>{l s='Payment experience within your store.' mod='payco'}</span></li>\
    <li><span>{l s='Your customers pay as guests without leaving your store.' mod='payco'}</span></li>\
</ul>";
...
var form_ticket = document.querySelector("#module_form_4 .panel .form-wrapper");
var form_ticket_group = document.querySelectorAll("#module_form_4 .panel .form-wrapper .form-group");

form_ticket_prepend.innerHTML = "<div class='row mp-pb-25'>\
    <div class='col-md-12'>\
        <h4 class='mp-title-checkout-body'>{l s='Your customer will make their purchase quickly, easily and safely with these settings:' mod='payco'}</h4>\
    </div>\
</div>";
...
var checkbox_ticket_prepend = "<div class='mp-all_checkbox'>\
    ...
    <label for='checkmeticket'><b class='mp-pointer mp-no-select mp-pl-5'>{l s='Select face payments' mod='payco'}</b></label>\
</div>";
...
var form_ticket_append = "<div class='panel-heading mp-panel-advanced-config'>\
    <i class='icon-cogs'></i> {l s='Advanced Configuration' mod='payco'}\
...
<div class='row text-ticket-advanced'>\
    <div class='col-md-12'>\
        <h4 class='mp-title-checkout-body mp-ticket-input-collapsible'>{l s='Activate other tools in our module ready to use.' mod='payco'}</h4>\
    </div>\
</div>";
...
var form_ticket_collapsible = document.querySelector("#module_form_4 .panel .mp-panel-advanced-config");
var form_ticket_collapsible_body = document.querySelectorAll(".mp-ticket-input-collapsible");
var form_ticket_collapsible_footer = document.querySelector("#module_form_4 .panel .panel-footer");
